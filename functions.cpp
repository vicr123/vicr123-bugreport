...
void Functions::log(QString content) {
    QTextStream str(stdout);
    str << "[" + QDateTime::currentDateTimeUtc().toString("hh:mm:ss") + "] \x1b[1m\x1b[37m[i] " + content + "\x1b[0m\n";
}

void Functions::err(QString content) {
    QTextStream str(stderr);
    str << "[" + QDateTime::currentDateTimeUtc().toString("hh:mm:ss") + "] \x1b[1m\x1b[31m[X] " + content + "\x1b[0m\n";
}

void Functions::good(QString content) {
    QTextStream str(stderr);
    str << "[" + QDateTime::currentDateTimeUtc().toString("hh:mm:ss") + "] \x1b[1m\x1b[32m[>] " + content + "\x1b[0m\n";
}

void Functions::warn(QString content) {
    QTextStream str(stderr);
    str << "[" + QDateTime::currentDateTimeUtc().toString("hh:mm:ss") + "] \x1b[1m\x1b[33m[!] " + content + "\x1b[0m\n";
}


QString Functions::getHTTPStatusCode(int code) {
    switch (code) {
        case 100:
            return "Continue";
        case 101:
...
QString Functions::getRandomString(int length) {
...
QString Functions::getRandomAlphanumericString(int length) {
    const QString possibleCharacters("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789");
...
QByteArray Functions::gzip(const QByteArray& data)
{
